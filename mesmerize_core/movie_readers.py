...
def hdf5_reader(path: str, var_name_hdf5="mov"):
    ...
    if extension in [
        ".n5",
        ".zarr",
    ]:  # Thankfully, the zarr library lines up closely with h5py past the initial open
        f = zarr.open(path, "r")
        ...
        try:
            ...
            if extension == ".mat":
                raise Exception(
                    f"Problem loading {path}: Unknown format. This may be in the original version 1 (non-hdf5) mat format; please convert it first"
                )
            else:
    ...
    ignore_keys = [
        "__DATA_TYPES__"
    ]  # Known metadata that tools provide, add to this as needed.
    ...
    Y = f.get(
        "acquisition/" + var_name_hdf5 + "/data"
        if extension == ".nwb"
        else var_name_hdf5
    )
    return Y
